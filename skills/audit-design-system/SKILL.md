...
description: Read-only audit of Figma nodes for design-system integration drift: missing shared components, local overrides, unbound tokens, variant drift. Use when reviewing Figma screens or URLs for DS alignment, checking tokens, or scoping fixes; output can be markdown report or JSON per user request.
---
...
- **Cursor chat**: Default to the human-readable markdown report unless the user or integration asks for JSON.
...
5. Present findings in the appropriate format based on the environment.
   Use JSON for Codex Desktop and machine-consumed review surfaces, markdown for Cursor chat, Claude Code CLI, and other chat-style environments (see Output Format Selection above).
