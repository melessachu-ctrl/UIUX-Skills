---
description: "HKTVmall Figma 設計預設套用 HKTVmall Design System。當任務涉及 HKTVmall UI/UX 設計、Figma 出稿、Prototype、或 uiux-design-studio 時自動啟用。"
alwaysApply: true
---
...
4. **產出標註**：Director Final Pack／UI Spec 須列出採用的 DS 名稱、fileKey、以及主要引用元件／token（未知則標「待盤點」並說明阻塞）。

## 與其他技能聯動

HKTVmall Figma 設計任務須**同時**遵守（本 repo 路徑）：

- `hktvmall-target-customers/SKILL.md`（客群／KPI）
- `ricky-design-guideline/SKILL.md`（R1–R9）
- `uiux-design-studio/SKILL.md`（若為編排流程）
